...
class OpenAICompatibleAdapter:
    ...
    def __init__(self, base_url: str, api_key: str, model_name: str, timeout: int = 60) -> None:
        if not base_url or not model_name:
            raise ModelError("URL e modelo são obrigatórios para o provedor compatível.")
    ...
    def generate(self, messages: Sequence[Message], system_prompt: str | None) -> ModelResponse:
        ...
        payload_messages.extend({"role": m.role.value, "content": m.content} for m in messages)
        payload = json.dumps(
            {"model": self.model_name, "messages": payload_messages, "stream": False}
        ).encode("utf-8")
        ...
        try:
            ...
            raise ModelError(f"Provedor retornou HTTP {exc.code}: {body}") from exc
        except (URLError, TimeoutError, json.JSONDecodeError) as exc:
            raise ModelError(f"Falha ao consultar o provedor: {exc}") from exc
        try:
            content = data["choices"][0]["message"]["content"]
        except (KeyError, IndexError, TypeError) as exc:
            raise ModelError("Resposta do provedor não segue o contrato esperado.") from exc
    ...
    def generate_stream(
        ...
        yield self.generate(messages, system_prompt).content
