    ...
    def publish(
        ...
        try:
            connection.execute(
                ...
                (
                    event["id"], event["topic"], event["sender"],
                    event["recipient"], json.dumps(payload, ensure_ascii=False),
                    event["created_at_ms"],
                ),
            )
            connection.commit()
    ...
    def recent(self, limit: int = 20) -> list[dict[str, Any]]:
        ...
        try:
            rows = connection.execute(
                "SELECT id, topic, sender, recipient, payload, created_at_ms "
        ...
        return [
            {
                "id": row[0], "topic": row[1], "sender": row[2],
                "recipient": row[3], "payload": json.loads(row[4]),
                "created_at_ms": row[5],
            }
            for row in rows
        ]
    ...
    def check(self, effect: str, approved: bool = False) -> PolicyDecision:
        ...
        if normalized in self.MEDIUM:
            return PolicyDecision(
                normalized, "medium", approved, not approved,
                "Operacao modifica estado e exige autorizacao." if not approved else "Autorizacao confirmada.",
            )
        if normalized in self.HIGH:
            return PolicyDecision(
                normalized, "high", approved, True,
                "Operacao sensivel exige autorizacao explicita." if not approved else "Autorizacao explicita confirmada.",
            )
            ...
            results.append(
                {
                    ...
                    "status": "available" if folder.exists() and source.exists() and manifest.exists() else "failed",
                }
            )
...
def doctor() -> dict[str, Any]:
    ...
    checks = {
        "king": True,
        "tyrion": health["failed"] == 0 and health["total"] == 23,
        "jhon": LOGS.exists(),
        "cyber": True,
        "organs": health,
    }
...
def main() -> int:
    ...
    if args.domain == "route":
        members = NedRouter().route(args.task)
        event = KingEventBus().publish("task.routed", {"task": args.task, "members": members}, sender="ned")
        print(json.dumps({"members": members, "event_id": event["id"]}, ensure_ascii=False, indent=2))
        return 0
    return 2
